...
export function Diff(props: Props) {
  ...
  return (
    <div class="text-xs overflow-x-scroll no-scrollbar">
      <div data-component="desktop" class="hidden">
        {rows().map((r) => (
          <div data-type={r.type} class="flex ">
            <div data-slot="before" data-diff-type={r.type === "removed" || r.type === "modified" ? "removed" : ""}>
              <Code code={r.left} lang={props.lang} flush />
            </div>
            <div data-slot="after" data-diff-type={r.type === "added" || r.type === "modified" ? "added" : ""}>
              <Code code={r.right} lang={props.lang} flush />
            </div>
          </div>
        ))}
      </div>
      <div>
        {mobileRows().map((block) => (
          <div data-type={block.type}>
            {block.lines.map((line) => (
              <div
                classList={{
                  "w-full": true,
                  "bg-error/40": block.type === "removed",
                  "bg-success/40": block.type === "added",
                }}
              >
                <Code code={line} lang={props.lang} flush />
              </div>
            ))}
          </div>
        ))}
